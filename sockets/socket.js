...
function isValidConversationId(conversationId) {
  ...
  return /^[a-f\d]{24}$/i.test(String(conversationId));
}
...
function addSocketToUser(io, userId, socketId) {
  ...
  if (!io.connectedUsers.has(normalizedUserId)) {
    io.connectedUsers.set(normalizedUserId, new Set());
  }

  io.connectedUsers.get(normalizedUserId).add(socketId);
}

function removeSocketFromUser(io, userId, socketId) {
  const normalizedUserId = normalizeId(userId);
  ...
  const sockets = io.connectedUsers.get(normalizedUserId);
  ...
  if (sockets.size === 0) {
    io.connectedUsers.delete(normalizedUserId);
    return true;
  }
...
function initializeSocket(server) {
  ...
  io.on("connection", (socket) => {
    ...
    try {
      ...
      console.error(
        "CALL SOCKET REGISTRATION ERROR:",
        error
      );
    }
    ...
    try {
      ...
      console.error(
        "LIVE SOCKET REGISTRATION ERROR:",
        error
      );
    }
